...
# MongoDB Atlas ODBC Install

RUN apt update
RUN apt -y install unixodbc
RUN apt -y install odbcinst
RUN apt install -y vim
RUN curl https://translators-connectors-releases.s3.amazonaws.com/mongosql-odbc-driver/ubuntu2204/1.0.1/release/mongoodbc.tar.gz --output mongoodbc.tar.gz
RUN tar -zxf mongoodbc.tar.gz --directory /usr/local/lib
